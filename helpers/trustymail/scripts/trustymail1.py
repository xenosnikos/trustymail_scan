...
"""trustymail1.py: A tool for scanning DNS mail records for evaluating security.
Usage:
  trustymail1.py (INPUT ...) [options]
  trustymail1.py (INPUT ...) [--output=OUTFILE] [--timeout=TIMEOUT] [--smtp-timeout=TIMEOUT] [--smtp-localhost=HOSTNAME] [--smtp-ports=PORTS] [--no-smtp-cache] [--mx] [--starttls] [--spf] [--dmarc] [--debug] [--json] [--dns=HOSTNAMES] [--psl-filename=FILENAME] [--psl-read-only]
  trustymail1.py (-h | --help)
...
Options:
  ...
  --smtp-localhost=HOSTNAME   The hostname to use when connecting to SMTP
                              ...
                              which trustymail1.py is being run.)
...
def main(scan_domain):
    ...
    # Monkey patching trustymail1.py to make it cache the PSL where we want
    if args["--psl-filename"] is not None:
        trustymail.PublicSuffixListFilename = args["--psl-filename"]
    # Monkey patching trustymail1.py to make the PSL cache read-only
